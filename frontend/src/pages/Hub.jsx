    ...
    <PageWrapper>
      ...
      <div className="bg-[#faf6e8] min-h-screen md:mt-[-25px] pt-28 pb-16 px-4 md:px-10 relative">
        ...
        {isSelectionMode && isAdmin && (
          <div className="sticky top-0 left-0 right-0 bg-white shadow-xl z-[100] p-4 px-6 md:px-20 flex justify-between items-center border-b-4 border-red-500 animate-slide-down">
          ...
          <div className="fixed inset-0 bg-black/90 z-[100] flex justify-center items-center overflow-y-auto p-4 md:p-8">
            <div className="bg-white rounded-2xl w-full overflow-y-auto md:overflow-hidden relative shadow-2xl flex flex-col md:flex-row max-h-[90vh]">
              ...
              <button
                ...
                className="fixed hidden md:block top-7 right-1 md:right-16 md:top-14 z-[70] text-gray-500 border border-gray-300 hover:text-black bg-white rounded-full p-1 shadow-md transition-colors"
              >
                <X size={24} />
              </button>
              ...
              <form onSubmit={handleCreatePost} className="flex flex-col gap-5">
                <div>
                  ...
                  <div className="flex gap-3">
                    <button type="button" onClick={() => setMediaType('photo')} className={`flex-1 py-2 px-3 rounded-lg border flex justify-center items-center gap-2 transition-colors ${mediaType === 'photo' ? 'bg-[#f0b000] border-[#f0b000] text-black font-semibold' : 'bg-gray-50 text-gray-600'}`}>
                      <ImageIcon size={18} /> Photo
                    </button>
                    <button type="button" onClick={() => setMediaType('video')} className={`flex-1 py-2 px-3 rounded-lg border flex justify-center items-center gap-2 transition-colors ${mediaType === 'video' ? 'bg-[#f0b000] border-[#f0b000] text-black font-semibold' : 'bg-gray-50 text-gray-600'}`}>
                      <Video size={18} /> Video
                    </button>
                    <button type="button" onClick={() => { setMediaType('text'); setFile(null); }} className={`flex-1 py-2 px-3 rounded-lg border flex justify-center items-center gap-2 transition-colors ${mediaType === 'text' ? 'bg-[#f0b000] border-[#f0b000] text-black font-semibold' : 'bg-gray-50 text-gray-600'}`}>
